...
parser.add_option("-t","--t",default = "unique",help="Test scenario")
parser.add_option("-r","--r",default = "ten",help="positive negative ratio")
parser.add_option("-s","--s",default = 3000,help="Number of steps")
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for i in range(num_steps):
            ...
            if i % 25 == 0 and verbose == True:
                print ("step {0} total and dtiloss {1} {2}".format(i,tloss,dtiloss ))
                ...
                print ("valid auc aupr {0} {1} test auc aupr {2} {3}".format(valid_auc, valid_aupr, test_auc, test_aupr))
    return best_valid_auc, best_valid_aupr, test_auc, test_aupr

test_auc_round = []
test_aupr_round = []
for r in range(10):
    print ("sample round {0}".format(r+1))
    if opts.t == 'o':
    ...
    if opts.t == 'unique':
        ...
        v_auc, v_aupr, t_auc, t_aupr = train_and_evaluate(DTItrain=DTItrain, DTIvalid=DTIvalid, DTItest=DTItest, graph=graph, num_steps=int(opts.s))
